...
# Ejecuta un prompt de limpieza/refactorización para Codex CLI.
# Uso:
#   ./ejecutar_limpieza_cartwise_codex.sh [ruta_del_repositorio_o_subcarpeta]
#
# Ejemplo:
#   ./ejecutar_limpieza_cartwise_codex.sh ~/proyectos/CartWise
#
# Si se ejecuta desde la raíz del repo y existe CartWise-Wireframes/,
# el script entra automáticamente a esa carpeta.

PROJECT_DIR="${1:-.}"

if ! command -v codex >/dev/null 2>&1; then
  echo "Error: no se encontró 'codex' en PATH."
  echo "Instala o configura Codex CLI antes de ejecutar este archivo."
  exit 1
fi

cd "$PROJECT_DIR"

if [ -d "CartWise-Wireframes" ]; then
  cd "CartWise-Wireframes"
fi

if [ ! -f "package.json" ]; then
  echo "Error: no encontré package.json en: $(pwd)"
  echo "Ejecuta el script desde la raíz del repo CartWise o desde CartWise-Wireframes."
  exit 1
fi
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  BRANCH_NAME="refactor/limpieza-webapp-$(date +%Y%m%d-%H%M)"
  CURRENT_BRANCH="$(git branch --show-current || true)"

  echo "Rama actual: ${CURRENT_BRANCH:-desconocida}"
  echo "Creando rama de trabajo: $BRANCH_NAME"
  git checkout -b "$BRANCH_NAME"
else
  echo "Aviso: esta carpeta no parece estar dentro de un repositorio git."
  echo "Continuaré, pero no se creará una rama de trabajo."
fi

mkdir -p .codex

PROMPT_FILE=".codex/cartwise_limpieza_webapp_prompt.md"
...
echo "Prompt escrito en: $PROMPT_FILE"
echo "Ejecutando Codex en modo no interactivo..."
echo
